...
- El sistema DEBERÁ resolver conflictos en campos de texto largo
  ...
  **Implementado**: `Note.content` y `daily_entries.content` son
  CRDT-backed (`github.com/Deln0r/ygo`, Go puro, wire-compatible con
  Yjs — no `yrs`/CGO como se planteó originalmente, ver `design.md`
  para el historial completo de la decisión). Validado end-to-end
  contra un contenedor real: dos ediciones concurrentes offline al
  mismo campo se mezclan sin perderse (`"AAA-0123456789-BBB"`, no una
  pisando a la otra).
