...
// Routes registers the note-related endpoints on r. Content lives on
// its own endpoint (POST .../content), separate from the LWW-governed
// fields below — see specs/arquitectura-inicial, "Resolución de
// conflictos".
...
func (req noteRequest) toNote(userID string) Note {
	...
	return Note{
		...
		Created: req.Created, Updated: req.Updated, Pinned: req.Pinned, UpdatedAt: req.UpdatedAt,
	}
}
...
func (h *Handler) postContent(w http.ResponseWriter, r *http.Request) {
	...
	stored, err := h.store.applyContentUpdate(r.Context(), id, userID, update, req.UpdatedAt)
	if err != nil {
		switch {
